...
import { formatMarketCap, formatPercent, formatPrice } from '../../lib/formatStats'
import { api } from '../../lib/api'
import type { StockStats as StockStatsData } from '@shared/types'
...
function StockCard({ stat, onExplainTerm, onSelectStock }: StockCardProps) {
  ...
  return (
    <div className="rounded-xl border border-hairline border-ink/10 bg-bone p-4 shadow-liftedSm dark:border-bone/10 dark:bg-bone/[0.04] dark:shadow-liftedSmDark">
      <div className="flex items-center justify-between gap-2">
        <button
          type="button"
          onClick={() => onSelectStock(stat)}
          className="min-w-0 rounded-md text-left focus-visible:ring-2 focus-visible:ring-vermilion"
        >
          <p className="font-display text-sm font-semibold text-cobalt underline decoration-cobalt/40 decoration-dotted underline-offset-2 hover:decoration-cobalt dark:text-cobalt-light dark:decoration-cobalt-light/40 dark:hover:decoration-cobalt-light">
            {stat.symbol}
          </p>
          <p className="truncate text-[11px] text-ink/55 dark:text-bone/55">{stat.name}</p>
        </button>
        <div className="shrink-0 text-right">
          <p className="font-display text-sm font-semibold tabular-nums text-ink dark:text-bone">
            {formatPrice(stat.price, stat.currency)}
          </p>
          <p
            className={`mt-1 inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[11px] font-medium tabular-nums ${
      ...
      <div className="mt-3 divide-y divide-ink/8 border-t border-hairline border-ink/10 dark:divide-bone/8 dark:border-bone/10">
  ...
  return (
    <div className="flex items-center justify-between gap-2 py-2">
      <div className="flex items-center gap-1.5">
        <span className="text-xs text-ink/60 dark:text-bone/60">{label}</span>
        <button
          type="button"
          onClick={onExplain}
          aria-label={`What does ${label} mean?`}
          className="rounded-full p-0.5 text-vermilion/70 transition-colors hover:bg-vermilion/10 hover:text-vermilion focus-visible:ring-2 focus-visible:ring-vermilion"
        >
          <Lightbulb size={12} aria-hidden="true" />
        </button>
      </div>
      <span className="font-display text-xs font-semibold tabular-nums text-ink dark:text-bone">{value}</span>
    </div>
  )
}

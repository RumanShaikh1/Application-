...
import { formatPercent, formatPrice } from '@renderer/lib/formatStats'
  ...
  return (
    <div className="animate-fade-in space-y-4">
      <button
        type="button"
        onClick={onBack}
        className="flex items-center gap-1.5 text-xs font-medium text-ink/60 transition-colors hover:text-vermilion focus-visible:ring-2 focus-visible:ring-vermilion"
      >
      ...
      <div className="flex items-center justify-between">
        <div>
          <p className="font-display text-lg font-semibold text-ink">{stat.symbol}</p>
          <p className="text-xs text-ink/55">{stat.name}</p>
        </div>
        <div className="text-right">
          ...
          <p
            className={`mt-0.5 inline-flex items-center gap-1 px-1.5 py-0.5 text-xs font-medium tabular-nums ${
      ...
      <section className="border border-hairline border-ink/15 bg-bone p-3.5">
        <h3 className="mb-2 font-mono text-xs font-semibold uppercase tracking-wide text-ink/55">More Insights</h3>
          ...
          {profile ? (
            <div className="space-y-2 text-xs text-ink/80">
              ...
              {profile.recommendationKey ? (
                <p>
                  <span className="text-ink/50">Analyst view: </span>
              ...
              {profile.profitMargins !== null ? (
                <p>
                  <span className="text-ink/50">Profit margin: </span>
      ...
      <section className="border border-hairline border-ink/15 bg-ink p-3.5">
        <div className="mb-2 flex items-center justify-between">
          <h3 className="font-mono text-xs font-semibold uppercase tracking-wide text-bone/60">Price Chart</h3>
          {chartVisible ? (
        ...
        {chartVisible ? (
          ...
          <button
            ...
            className="w-full rounded-full border border-hairline border-bone/25 py-2 text-xs font-medium text-bone/70 transition-colors hover:border-vermilion/60 hover:text-vermilion focus-visible:ring-2 focus-visible:ring-vermilion"
          >
            Show price chart
          </button>
        )}
      ...
      <section className="border border-hairline border-ink/15 bg-bone p-3.5">
        <h3 className="mb-2 flex items-center gap-1.5 font-mono text-xs font-semibold uppercase tracking-wide text-ink/55">
        ...
        >
            ...
            {news.map((item) => (
              <li key={item.link}>
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="group flex items-start gap-1.5 text-xs text-cobalt transition-opacity hover:opacity-75"
                >
                  <ExternalLink size={11} aria-hidden="true" className="mt-0.5 shrink-0" />
                  <span>
                    {item.title}
                    <span className="ml-1 text-ink/50">— {item.publisher}</span>
                  </span>
                </a>
              </li>
            ))}
          ...
          {contextState === 'loading' ? <p className="mt-3 text-[11px] text-ink/50">Thinking...</p> : null}
          {contextState === 'error' ? (
            <div className="mt-3 space-y-1.5">
              <p className="text-[11px] text-ink">{contextError}</p>
              <button
                type="button"
                onClick={requestContext}
          ...
          {contextState === 'done' ? (
            <p className="mt-3 border border-hairline border-ink/12 bg-ink/[0.03] p-2.5 text-sm leading-relaxed text-ink/80">
              {context}
            </p>
          ) : null}
        </StatusState>

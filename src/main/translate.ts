import type { StockContextRequest, TranslateRequest } from '../shared/ipc-channels'
...
async function callGemini(systemPrompt: string, userMessage: string, temperature: number): Promise<string> {
  ...
  if (!apiKey) {
    throw new Error('GEMINI_API_KEY is not set. Add it to a .env file in the project root to enable translations.')
  }

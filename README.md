...
launcher/   standalone MarketPane.exe-style desktop app (WinForms/PowerShell) that links a browser and starts the server
            ...
            launcher/MarketPane.Desktop/prebuilt/MarketPane.Desktop.exe is a single-window alternative -
            no separate browser, but the Jargon Buster extension can't run inside it (see below)
```
...
**Prefer one single app window instead of a separate browser?** Double-click `launcher/MarketPane.Desktop/prebuilt/MarketPane.Desktop.exe` - a pre-built, self-contained, compressed single `.exe` (~70MB - a plain committed file, no Git LFS involved, so it downloads correctly however you got the repo, including GitHub's "Download ZIP" button). It embeds the whole web app in one real window - no browser tab, no address bar - and needs nothing pre-installed except Node.js, not even the .NET runtime. Same auto-install behavior as `MarketPane.bat` above, plus: if the WebView2 Runtime isn't already on your machine (it ships with Windows 10/11 in almost all cases, so this is rare), it downloads and silently installs Microsoft's official small bootstrapper for it automatically - a real Windows security (UAC) prompt during that specific step is expected, not a bug. The trade-off: the Jargon Buster browser extension can't be loaded into an embedded WebView2 window the way it can into a real Chrome/Edge tab, so this option is Decision Replay/Simulator/Tax Understanding only, not the extension. (To rebuild it yourself after editing the source: `dotnet publish -c Release -r win-x64 --self-contained true -p:PublishSingleFile=true -p:IncludeNativeLibrariesForSelfExtract=true -p:EnableCompressionInSingleFile=true` from inside `launcher/MarketPane.Desktop/`, needs the .NET 8 SDK - the compression flag is what keeps it under GitHub's 100MB push limit, ~156MB uncompressed vs. ~70MB with it.)
